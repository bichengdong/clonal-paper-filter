...
async function gistRead() {
  ...
  try {
    ...
    if (e.name === 'AbortError') setSS('yellow','同步超时（离线模式）');
    else setSS('red','同步失败: '+e.message);
    return null;
  }
}
...
async function gistWrite(data) {
  const id = S.gistCfg.id || GIST_ID, tok = S.gistCfg.token || GIST_TOKEN;
  if (!id || !tok) { setSS('yellow','未配置Token，数据仅本地保存'); return false; }
  setSS('yellow','保存中…');
  try {
    const controller = new AbortController();
    ...
    if (e.name === 'AbortError') setSS('yellow','保存超时（数据已本地保存）');
    else setSS('red','保存失败: '+e.message);
    return false;
  }
}
