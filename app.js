...
async function gistRead() {
  ...
  } catch(e) { setSS('red','同步失败: '+e.message); return null; }
}
...
async function gistWrite(data) {
  ...
  try {
    ...
    if (!r.ok) throw new Error('HTTP '+r.status);
    setSS('green','已保存 '+new Date().toLocaleTimeString());
    return true;
  } catch(e) { setSS('red','保存失败: '+e.message); return false; }
}
...
async function loadPapers(pg, type) {
  ...
  document.getElementById('plist').innerHTML='<div class="empty"><div class="ei"><span class="spin"></span></div></div>';
  try {
    const r=await fetch('data/'+S.confType+'_'+pg+'.json');
    const d=await r.json();
    S.page=pg; S.totalPages=d.total_pages; S.papers=d.papers;
    renderPapers(); updatePager();
  } catch(e) {
    document.getElementById('plist').innerHTML='<div class="empty"><div class="ei">⚠️</div><p>加载失败</p></div>';
  }
}
...
document.getElementById('save-gist').onclick=async()=>{
  ...
  updGistDot(); toast('Gist配置已保存，测试中…','info');
  const r=await gistRead();
  toast(r!==null?'✅ 连接成功':'❌ 连接失败，请检查 ID 和 Token',r!==null?'success':'error');
};
